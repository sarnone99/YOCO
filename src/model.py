"""YOLO_v3/YOCO Model Defined in Keras."""
...
from keras.layers import Conv2D, Add, ZeroPadding2D, UpSampling2D, Concatenate, MaxPooling2D, BatchNormalization, Flatten, Dense, Dropout, LeakyReLU
...
from clustering import npAggloCluster
...
def yolo_eval_all(yolo_outputs,
              anchors,
              num_classes,
              image_shape,
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=False):
    ...
    # loss = tf.where(domain == 0, loss, 0.0)
    return loss
...
def cluster_embeddings(x):
    x = tf.numpy_function(npAggloCluster, x, [tf.float32, tf.float32, tf.float32])
    return x
...
def create_model(input_shape, anchors, num_classes, weights_path=None, inference_only=False):
    ...
    x = compose(
            ...
            UpSampling2D(2))(x1)
    x = Concatenate()([x,darknet.layers[152].output])
    x2, y2 = make_last_layers(x, 256, num_anchors//3*(num_classes+5))
    x = compose(
            DarknetConv2D_BN_Leaky(128, (1,1)),
            UpSampling2D(2))(x2)
    x = Concatenate()([x,darknet.layers[92].output])
    ...
    ''' Image Discriminator '''
    x = GradientReversal()(darknet(domain_input))
    ...
    x = Flatten()(x)
    x = Dense(1)(x)
    instDisc3 = Model(y3_mean_input, x, name='inst3')
    instDisc3_out = instDisc3(y3_mean)

    ''' YOLO loss '''
    yolo_detection_loss = Lambda(yolo_loss, output_shape=(1,), name='yolo',
